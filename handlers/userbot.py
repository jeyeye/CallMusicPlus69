...
async def ubkickme(_, message: Message):
  ...
  try:
    ...
    await i_go_away.edit("`Successfully Leaved This Shit Chat!`")
  except Exception as lol:
...
async def getlogs(client: NEXAUB, message: Message, app_):
  ...
  if status == "on":
    ...
    try:
      ...
      pmchat = await NEXAUB.create_group(f"Call Userbot's PM Logs", BOT_OWNER)
      chat_id = pmchat.id
